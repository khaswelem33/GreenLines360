...
/**
 * Mobile navigation drawer with accordion behavior.
 *
 * IMPORTANT: the drawer is rendered through a portal on document.body.
 * The sticky header uses backdrop-blur (a CSS backdrop-filter), which
 * makes the header the containing block for `position: fixed`
 * descendants — rendering the drawer inside the header clips it to the
 * header's height. The portal keeps the drawer fixed to the viewport.
 */
      ...
      ? createPortal(
          <div className="fixed inset-0 z-[100] h-[100dvh]">
            ...
            <button
              ...
              onClick={close}
              className="absolute inset-0 h-full w-full bg-charcoal/50 backdrop-blur-sm"
            />
            ...
            <div
              ...
              className="absolute inset-y-0 end-0 flex h-full w-[22rem] max-w-[90vw] flex-col bg-ivory-light shadow-lifted"
            >
              {/* Top bar (fixed height) */}
              <div className="flex shrink-0 items-center justify-between border-b border-neutral-border px-5 py-4">
              ...
              {/* Scrollable body */}
              <nav className="flex flex-1 flex-col overflow-y-auto px-5 pb-10 pt-4">
                      ...
                      >
                        <button
                          ...
                          className={cn(
                            ...
                            "rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
                          )}
                        >
                          {item.label}
                          <ChevronDown
                ...
                <ul className="mt-2 grid shrink-0 grid-cols-2 gap-1">
                  {locales.map((locale) => {
